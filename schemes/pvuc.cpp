...
void pvuc(vector <edge> &E, vector <cell> &convol)
{
    ...
    for (i=0;i<numEdge;i++)
    {
        ...
        u_plus = (convol[parentcellid].norm_prim_var[1] + fabs(convol[parentcellid].norm_prim_var[1]))/2.0;
        u_minus = (convol[neighbourcellid].norm_prim_var[1] - fabs(convol[neighbourcellid].norm_prim_var[1]))/2.0;
        ...
        */
        ...
        double u_average;
        double u_plus, u_minus;
        u_average = (convol[parentcellid].norm_prim_var[1] + convol[neighbourcellid].norm_prim_var[1])/2.0;
        u_plus = (u_average + fabs(u_average))/2.0;
        u_minus = (u_average - fabs(u_average))/2.0;
        ...
        */
        ...
        // pressure flux
        double pcell_sound_speed, ncell_sound_speed, avg_sound_speed;
        ...
        temp2 = ((gamma + 1.0)/(4.0*(gamma - 1.0)))*sqrt(2.0/(gamma*pi))*((convol[parentcellid].norm_prim_var[3]*powf(pcell_sound_speed,2))/(2.0*avg_sound_speed));
        ...
        temp5 = -((gamma + 1.0)/(4.0*(gamma - 1.0)))*sqrt(2.0/(gamma*pi))*((convol[neighbourcellid].norm_prim_var[3]*powf(ncell_sound_speed,2))/(2.0*avg_sound_speed));
