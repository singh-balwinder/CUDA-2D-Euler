...
void mpvu(vector <edge> &E, vector <cell> &convol)
{
    ...
    for (i=0;i<numEdge;i++)
    {
        ...
        */
        ...
        p_positive_flux[0] = (convol[parentcellid].norm_prim_var[0]/(2.0*sqrt(pi*parent_beta)))*parent_phi;
        p_negative_flux[0] = (-convol[neighbourcellid].norm_prim_var[0]/(2.0*sqrt(pi*neighbour_beta)))*neighbour_phi;
        p_positive_flux[1] = (convol[parentcellid].norm_prim_var[3]/2.0) + (((convol[parentcellid].norm_prim_var[0]/(2.0*sqrt(pi*parent_beta)))*(convol[parentcellid].norm_prim_var[1]))*parent_phi);
        p_negative_flux[1] = (convol[neighbourcellid].norm_prim_var[3]/2.0) - (((convol[neighbourcellid].norm_prim_var[0]/(2.0*sqrt(pi*neighbour_beta)))*(convol[neighbourcellid].norm_prim_var[1]))*neighbour_phi);
        p_positive_flux[2] = ((convol[parentcellid].norm_prim_var[0]/(2.0*sqrt(pi*parent_beta)))*(convol[parentcellid].norm_prim_var[2]))*parent_phi;
        p_negative_flux[2] = -((convol[neighbourcellid].norm_prim_var[0]/(2.0*sqrt(pi*neighbour_beta)))*(convol[neighbourcellid].norm_prim_var[2]))*neighbour_phi;
        ...
        temp3 = (((convol[parentcellid].norm_prim_var[0]*powf(convol[parentcellid].norm_prim_var[1],2))/(4.0*sqrt(pi*parent_beta)))*parent_phi) + (((convol[parentcellid].norm_prim_var[0]*powf(convol[parentcellid].norm_prim_var[2],2))/(4.0*sqrt(pi*parent_beta)))*parent_phi);
        temp4 = (convol[parentcellid].norm_prim_var[3]/(2.0*sqrt(pi*parent_beta)))*powf(parent_phi,2);
        p_positive_flux[3] = temp1 + temp2 + temp3 +temp4;
        ...
        temp7 = -(((convol[neighbourcellid].norm_prim_var[0]*powf(convol[neighbourcellid].norm_prim_var[1],2))/(4.0*sqrt(pi*neighbour_beta)))*neighbour_phi) - (((convol[neighbourcellid].norm_prim_var[0]*powf(convol[neighbourcellid].norm_prim_var[2],2))/(4.0*sqrt(pi*neighbour_beta)))*neighbour_phi);
        temp8 = -(convol[neighbourcellid].norm_prim_var[3]/(2.0*sqrt(pi*neighbour_beta)))*powf(neighbour_phi,2);
        p_negative_flux[3] = temp5 + temp6 + temp7 + temp8;
